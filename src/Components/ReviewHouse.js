...
const ReviewHouse = ({ setSelectedIndex, homeData }) => {
  const from = format(new Date(homeData?.from), 'PP').split(',')[0].split(' ');
  const to = format(new Date(homeData?.To), 'PP').split(',')[0].split(' ');
  ...
  return (
    <>
      ...
      <h3 className='text-gray-900 title-font text-xl font-medium'>
        {homeData?.totalNights} nights in <span className='uppercase'>{homeData?.location}</span>
      </h3>
