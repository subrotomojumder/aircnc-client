...
const HomeDetails = () => {
  ...
  const { location, title, from, To, price, total_guest, bedrooms, bathrooms, description, image, host } = homeDetails;
  const { name, image: hostImage, email } = host;
  const totalNights = differenceInCalendarDays(
    new Date(To),
    new Date(from)
  )

  let sub_total = parseFloat(price) * totalNights;
      ...
      <div className='md:flex justify-between sm:mx-10 md:mx-20 px-4 lg:mx-40 py-12'>
        ...
        <div className='p-4 md:w-1/2 lg:w-1/3 w-full h-full rounded shadow-lg'>
          <h1 className='text-gray-900 text-3xl title-font font-medium mb-2'>
            ${price}/ <span className='font-thin'>night</span>
          </h1>
          <div className='flex gap-1 mb-2'>
            <StarIcon className='h4 w-4 text-green-500' />{' '}
            <span>4.8 (10 reviews)</span>
          ...
          <div className='flex justify-between items-center p-2 border mt-1 mb-2'>
            <div>{format(new Date(from), 'PP')}</div>
            <div>
              <ArrowRightIcon className='h5 w-5' />
            </div>
            <div>{format(new Date(To), 'PP')}</div>
          </div>

          <div className='flex border-t border-gray-200 py-2'>
            <span className='text-gray-500'>${price} x {totalNights} nights</span>
          ...
          <div className='mt-6 mb-2' onClick={handleReserve}>
            <PrimaryButton
              type='submit'
